...
import (
    ...
    enginepkg "order-matching-engine/src/engine"
)

// setup a new engine for each test
func setupEngine() *enginepkg.MatchingEngine {
    return enginepkg.NewMatchingEngine()
}

// Helper to create a test order
// We use a manual timestamp to control FIFO order
func newTestOrder(id, symbol string, side enginepkg.Side, oType enginepkg.OrderType, price, quantity int64, ts int64) *enginepkg.Order {
    return &enginepkg.Order{
        ID:             id,
        Symbol:         symbol,
        Side:           side,
        Type:           oType,
        ...
        Quantity:       quantity,
        FilledQuantity: 0,
        Status:         enginepkg.StatusAccepted,
        Timestamp:      ts,
    }
}
...
func TestExample1_SimpleFullMatch(t *testing.T) {
    ...
    sellOrder := newTestOrder("order-001", "AAPL", enginepkg.Sell, enginepkg.Limit, 15050, 1000, 1000)
    buyOrder1 := newTestOrder("order-002", "AAPL", enginepkg.Buy, enginepkg.Limit, 15045, 500, 1001)
    ...
    newBuyOrder := newTestOrder("order-new", "AAPL", enginepkg.Buy, enginepkg.Limit, 15050, 500, 1002)
    ...
    assert.Equal(enginepkg.StatusFilled, newBuyOrder.Status)
    ...
    assert.NoError(err)
    assert.Equal(enginepkg.StatusPartialFill, status.Status)
...
func TestExample2_MultiplePriceLevels(t *testing.T) {
    ...
    sell1 := newTestOrder("order-003", "AAPL", enginepkg.Sell, enginepkg.Limit, 15050, 300, 1000)
    sell2 := newTestOrder("order-004", "AAPL", enginepkg.Sell, enginepkg.Limit, 15052, 400, 1001)
    sell3 := newTestOrder("order-005", "AAPL", enginepkg.Sell, enginepkg.Limit, 15055, 600, 1002)
    buy1 := newTestOrder("order-006", "AAPL", enginepkg.Buy, enginepkg.Limit, 15045, 500, 1003)
    ...
    newBuyOrder := newTestOrder("order-new", "AAPL", enginepkg.Buy, enginepkg.Limit, 15053, 800, 1004)
    resp, err := eng.SubmitOrder(newBuyOrder)
    ...
    assert.Equal(enginepkg.StatusPartialFill, newBuyOrder.Status)
...
func TestExample3_TimePriorityFIFO(t *testing.T) {
    ...
    sell1 := newTestOrder("order-007", "AAPL", enginepkg.Sell, enginepkg.Limit, 15050, 200, 1000) // First
    sell2 := newTestOrder("order-008", "AAPL", enginepkg.Sell, enginepkg.Limit, 15050, 300, 1001) // Second
    sell3 := newTestOrder("order-009", "AAPL", enginepkg.Sell, enginepkg.Limit, 15050, 400, 1002) // Third
    ...
    newBuyOrder := newTestOrder("order-new", "AAPL", enginepkg.Buy, enginepkg.Limit, 15050, 500, 1003)
    resp, err := eng.SubmitOrder(newBuyOrder)
    ...
    assert.Equal(enginepkg.StatusAccepted, status9.Status)
...
func TestExample4_MarketOrderExecution(t *testing.T) {
    ...
    sell1 := newTestOrder("order-010", "AAPL", enginepkg.Sell, enginepkg.Limit, 15050, 200, 1000)
    sell2 := newTestOrder("order-011", "AAPL", enginepkg.Sell, enginepkg.Limit, 15052, 300, 1001)
    sell3 := newTestOrder("order-012", "AAPL", enginepkg.Sell, enginepkg.Limit, 15055, 400, 1002)
    ...
    newBuyOrder := newTestOrder("order-new", "AAPL", enginepkg.Buy, enginepkg.Market, 0, 600, 1003)
    resp, err := eng.SubmitOrder(newBuyOrder)
    ...
    assert.Equal(enginepkg.StatusPartialFill, status12.Status)
}
...
func TestExample5_InsufficientLiquidity(t *testing.T) {
    ...
    sell1 := newTestOrder("order-013", "AAPL", enginepkg.Sell, enginepkg.Limit, 15050, 100, 1000)
    buy1 := newTestOrder("order-014", "AAPL", enginepkg.Buy, enginepkg.Limit, 15045, 500, 1001)

    _, _ = eng.SubmitOrder(sell1)
    _, _ = eng.SubmitOrder(buy1)

    // 2. New Market Order Arrives [cite: 248-253]
    newBuyOrder := newTestOrder("order-new", "AAPL", enginepkg.Buy, enginepkg.Market, 0, 500, 1003)
    resp, err := eng.SubmitOrder(newBuyOrder)
...
func TestCancelOrder(t *testing.T) {
    ...
    // 1. Add an order
    order := newTestOrder("order-tocancel", "AAPL", enginepkg.Buy, enginepkg.Limit, 10000, 100, 1000)
    ...
    assert.NoError(err)
    assert.Equal(enginepkg.StatusAccepted, status.Status)

    // 3. Cancel the order
    cancelledOrder, err := eng.CancelOrder("order-tocancel")
    assert.NoError(err)
    assert.Equal(enginepkg.StatusCancelled, cancelledOrder.Status)
    ...
    status, err = eng.GetOrderStatus("order-tocancel")
    assert.NoError(err)
    assert.Equal(enginepkg.StatusCancelled, status.Status)
    ...
    _, _ = eng.SubmitOrder(newTestOrder("sell-order", "AAPL", enginepkg.Sell, enginepkg.Limit, 10000, 100, 1001))
